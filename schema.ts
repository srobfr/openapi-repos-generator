  ...
  return propDef.anyOf
    ...
    ? (() => {
      ...
      addImportBy(
        rootPropDef,
        schemaName,
        `src/api/schemas/${schemaName}Schema`,
      );
      return schemaName;
    })()
    : (() => {
...
const buildRecursiveSchemaCode = (
  ...
  return `${buildImports([schema, ...Object.values(schema.propertiesByName).map(p => p.propDef)], `src/api/schemas/${schema.zodName}`)}
